...
class FCOS(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.object_sizes_of_interest = cfg.MODEL.POTO.OBJECT_SIZES_OF_INTEREST
        self.distance_topk = cfg.MODEL.POTO.DISTANCE_TOPK
        ...
        for shifts_per_image, targets_per_image in zip(shifts, targets):
            ...
            for stride, shifts_i in zip(self.fpn_strides, shifts_per_image):
                ...
                _, topk_idxs = distances.topk(
                    self.distance_topk, dim=1, largest=False)
            ...
            is_foreground = torch.zeros_like(
                is_cared_in_the_level
            ).scatter_(1, candidate_idxs, True)
